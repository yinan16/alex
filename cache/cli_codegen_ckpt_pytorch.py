...
class Model(torch.nn.Module):
    ...
    @staticmethod
    def model(data_block_input_data, training, probes, trainable_params):
    ...
    @staticmethod
    def get_loss(trainable_params, data_block_labels, model_block_output):
...
C = Checkpoint("examples/configs/small1.yml", ['checkpoints', 'test.json'], None)
...
def inference(data_block_input_data, probes, trainable_params):
    ...
    preds = model(data_block_input_data, training, probes, trainable_params)
    
    return preds
    
def evaluation(data_block_input_data, probes, data_block_labels, trainable_params):
    ...
    loss = model.get_loss(trainable_params, data_block_labels, preds)
    return loss
    
    
def train(data_block_input_data, probes, data_block_labels, trainable_params):
...
def loop(val_inputs, data_block_labels, trainable_params, probes, data_block_val_labels):
    ...
    for epoch in range(90):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            train(inputs, probes, data_block_labels, trainable_params)
            optimizer.step()
    
            if i % 500 == 499:
                results = evaluation(val_inputs, probes, data_block_val_labels, trainable_params)
                print(results)
