...
class Model(torch.nn.Module):
    ...
    def forward(self, trainable_params, data_block_input_data):
        x = self.model(trainable_params, data_block_input_data)
        return x
    ...
    @staticmethod
    def get_loss(trainable_params, data_block_input_data, training, model_block_output):
...
def evaluation(trainable_params, data_block_input_data):
    ...
    loss = model.get_loss(trainable_params, data_block_input_data, training, preds)
    return loss
...
def loop(trainable_params, val_inputs):
...
loop(trainable_params, val_inputs)
