...
class Model(torch.nn.Module):
    ...
    @staticmethod
    def model(probes, training, data_block_input_data, trainable_params):
...
def inference(probes, data_block_input_data, trainable_params):
    ...
    preds = model(probes, training, data_block_input_data, trainable_params)
...
def train(probes, model_block_d_1, trainable_params, data_block_labels, data_block_input_data):
...
def loop(probes, model_block_d_1, trainable_params, data_block_labels, data_block_val_labels, val_inputs):
    ...
    for epoch in range(90):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            train(probes, model_block_d_1, trainable_params, data_block_labels, inputs)
            optimizer.step()
    
            if i % 500 == 499:
                results = evaluation(probes, model_block_d_1, trainable_params, data_block_val_labels, val_inputs)
                print(results)
...
loop(probes, model_block_d_1, trainable_params, data_block_labels, data_block_val_labels, val_inputs)
