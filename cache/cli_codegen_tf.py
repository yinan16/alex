...
def get_trainable_params():
    ...
    model_block_dense_20ue_weights_initializer_xavier_uniform = tf.keras.initializers.glorot_uniform(seed=2)(shape=[16384, 10])
...
def model(data_block_input_data, trainable_params, training):
    ...
    model_block_flatten_18so = tf.reshape(tensor=model_block_conv_16qy, shape=(-1, tf.math.reduce_prod(tf.convert_to_tensor([32, 32, 16]))), name='model_block/flatten_18so')
