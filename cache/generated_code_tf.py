...
def get_trainable_params(ckpt=None):
    ...
    conv_15pq_filters_initializer_xavier_uniform = tf.keras.initializers.glorot_uniform(seed=1)(shape=(5, 5, 16, 16))
    ...
    dense_19tw_weights_initializer_xavier_uniform = tf.keras.initializers.glorot_uniform(seed=2)(shape=[16384, 10])
...
def model(input_data, trainable_params, training):
    ...
    flatten_17rg = tf.reshape(tensor=conv_15pq, shape=(-1, tf.math.reduce_prod(tf.convert_to_tensor([32, 32, 16]))), name='flatten_17rg')
